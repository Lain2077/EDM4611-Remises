...
# Returns 4 results for searching the string "hello" in the library
        c = search(str(op('userField')[0, 0]))
        print(c)
        print(c[2].hexagon, c[2].wall, c[2].shelf, c[2].volume, c[2].page, c[2].position)

        op('bookLoc')[0, 0] = c[2].hexagon
        op('bookLoc')[0, 1] = c[2].wall
        op('bookLoc')[0, 2] = c[2].shelf
        op('bookLoc')[0, 3] = c[2].volume
        op('bookLoc')[0, 4] = c[2].page
        op('bookLoc')[0, 5] = c[2].position
        op('searchDone').par.triggerpulse.pulse()
    return
